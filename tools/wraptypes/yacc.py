...
class Parser:
    ...
    def __init__(self):
        ...
        self.action = dict()  # LR Action table
        self.goto = dict()  # LR goto table
        self.require = dict()  # Attribute require table
        self.method = "Unknown LR"  # Table construction method used

        # <ah> 25 Jan 2007
        self.statestackstack = list()
        self.symstackstack = list()
    ...
    def parse(self, input=None, lexer=None, debug=0):
        ...
        statestack = list()  # Stack of parsing states
        self.statestack = statestack
        symstack = list()  # Stack of grammar symbols
                ...
                if t < 0:
                    ...
                    if plen:
                        ...
                        try:
                            ...
                            sym.endlineno = getattr(targ[-1], "endlineno",
                                                    targ[-1].lineno)
                            sym.lexpos = targ[1].lexpos
                            sym.endlexpos = getattr(targ[-1], "endlexpos",
                                                    targ[-1].lexpos)
            ...
            if t == None:
                ...
                if not self.errorcount:
                    ...
                    if self.errorfunc:
                        ...
                        # Set some special functions available in error
                        # recovery
                        errok = self.errok
                        token = get_token
...
def validate_dict(d):
    for n, v in list(d.items()):
        ...
        if 1 and isinstance(v,
                            types.FunctionType) and v.__code__.co_argcount == 1:
            try:
                doc = v.__doc__.split(" ")
                if doc[1] == ':':
                    sys.stderr.write(
...
def initialize_vars():
    ...
    Prodmap = dict()  # A dictionary that is only used to detect duplicate
    # productions.
    ...
    First = dict()  # A dictionary of precomputed FIRST(x) symbols

    Follow = dict()  # A dictionary of precomputed FOLLOW(x) symbols

    # Precedence rules for each terminal. Contains tuples of the
    Precedence = dict()
    # form ('right',level) or ('nonassoc', level) or ('left',level)

    LRitems = list()  # A list of all LR items for the grammar.  These are the
...
def augment_grammar(start=None):
    ...
    Productions[0] = Production(name="S'", prod=[start], number=0, len=1,
                                prec=('right', 0), func=None)
...
def lr_init_vars():
    ...
    _lr_action = dict()  # Action table
    _lr_goto = dict()  # Goto table
...
def compute_lookback_includes(C, trans, nullable):
    lookdict = dict()  # Dictionary of lookback relations
    includedict = dict()  # Dictionary of include relations
...
def compute_follow_sets(ntrans, readsets, inclsets):
    ...
    R = lambda x: inclsets.get(x, list())
    F = digraph(ntrans, R, FP)
    return F
    ...
    for trans, lb in list(lookbacks.items()):
        ...
        for state, p in lb:
            if state not in p.lookaheads:
                p.lookaheads[state] = list()
            f = followset.get(trans, list())
            for a in f:
...
def lr_parse_table(method):
    ...
    actionp = dict()  # Action production array (temporary)
...
def yacc(method=default_lr, debug=yaccdebug, module=None, tabmodule=tab_module,
         ...
         debugfile=debug_file, outputdir='', parserclass=Parser):
    global yaccdebug
    yaccdebug = debug
    ...
    if (optimize and lr_read_tables(tabmodule, 1)):
        ...
        symbols = [ldict[f] for f in list(ldict.keys())
                   if (
            type(ldict[f]) in (types.FunctionType, types.MethodType) and ldict[
                f].__name__[
                :2] == 'p_'
        ...
        if not lr_read_tables(tabmodule):
            ...
            otherfunc = [ldict[f] for f in list(ldict.keys())
                         if (
                type(f) in (types.FunctionType, types.MethodType) and ldict[
                    f].__name__[
                    :2] != 'p_')]
