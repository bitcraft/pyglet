        ...
        def print_link_modules_imports(self):
            for name in self.link_modules:
                ...
                def print_epilogue(self):
                    ...
                    print('\n'.join(textwrap.wrap(
                        '__all__ = [%s]' % ', '.join(
                            [repr(n) for n in self.all_names]),
                        width=78,
                        break_long_words=False)), file=self.file)

                    def handle_ctypes_constant(self, name, value, filename,
                                               lineno):
                        if self.does_emit(name, filename):
                            print('%s = %r' % (name, value),
                                  end=' ', file=self.file)
                            print(
                                '\t# %s:%d' % (filename, lineno), file=self.file)
                    ...
                    def handle_ctypes_type_definition(self, name, ctype,
                        ...
                        def emit_type(self, t):
                            ...
                            for s in t.get_required_type_names():
                                if s in self.known_types:
                                    if s in self.linked_symbols:
                                        print(
                                            '%s = %s' % (
                                                s, self.linked_symbols[s]),
                                            file=self.file)
                                        else:
                                        s_ctype, s_filename, s_lineno = \
                                            self.known_types[s]
                                        s_ctype.visit(self)

                                        self.emit_type(s_ctype)
                                        print('%s = %s' % (s, str(s_ctype)),
                                              end=' ', file=self.file)
                                        print(
                                            '\t# %s:%d' % (
                                                s_filename, s_lineno),
                            ...
                            def visit_struct(self, struct):
                                ...
                                base = {True: 'Union', False: 'Structure'}[
                                    struct.is_union]
                                print(
                                    'class struct_%s(%s):' % (
                                        struct.tag, base),
                                ...
                                if not struct.opaque:
                                    ...
                                    print('struct_%s._fields_ = [' % struct.tag,
                                          file=self.file)
                                    if struct.opaque:
                                        print("    ('_opaque_struct', c_int)",
                                              file=self.file)
                                        ...
                                        def visit_enum(self, enum):
                                            ...
                                            def handle_ctypes_function(self,
                                                                       name,
                                                                       restype,
                                                                       argtypes,
                                                                       filename,
                                                                       lineno):
                                                if self.does_emit(name,
                                                                  filename):
                                                ...
                                                def handle_ctypes_variable(self,
                                                                           name,
                                                                           ctype,
                                                                           filename,
                                                                           lineno):
                                            ...
                                            def main(*argv):
                                                ...
                                                op.add_option('-D', '--define',
                                                              dest='defines',
                                                              default=list(),
                                                              help='define token NAME=VALUE',
                                                              action='append')
                                                op.add_option('-i',
                                                              '--include-file',
                                                              action='append',
                                                              dest='include_files',
                                                              help='assume FILE is iincluded',
                                                              metavar='FILE',
                                                              default=list())
                                                op.add_option('-I',
                                                              '--include-dir',
                                                ...
                                                op.add_option('-a',
                                                              ...
                                                              action='store_true',
                                                              dest='all_headers',
                                                              help='include symbols from all headers',
                                                              default=False)
                                                ...
                                                for define in options.defines:
                                                    name, value = define.split(
                                                        '=')
                                                    wrapper.preprocessor_parser.define(
                                                        name, value)
