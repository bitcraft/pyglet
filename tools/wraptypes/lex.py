    ...
    def writetab(self, tabfile):
        ...
        for key, lre in list(self.lexstatere.items()):
            ...
            for i in range(len(lre)):
                titem.append(
                    (self.lexstateretext[key][i], _funcs_to_names(lre[i][1])))
            tabre[key] = titem
        ...
        while lexpos < lexlen:
            ...
            for lexre, lexindexfunc in self.lexre:
                ...
                if not self.lexoptimize:
                    ...
                    # 20/Jan/2007
                    if newtok.type not in self.lextokens and len(
                            newtok.type) > 1:
                        raise LexError(
...
def lex(module=None, object=None, debug=0, optimize=0, lextab="lextab",
        ...
        if not (isinstance(states, tuple) or isinstance(states, list)):
            ...
            for s in states:
                if not isinstance(s, tuple) or len(s) != 2:
                    print(
                        "lex: invalid state specifier %s. Must be a tuple (statename,'exclusive|inclusive')" % repr(
                            s))
                    error = 1
                    continue
                name, statetype = s
    ...
    funcsym = dict()  # Symbols defined as functions
    strsym = dict()  # Symbols defined as strings
    toknames = dict()  # Mapping of symbols to token names

    for s in list(stateinfo.keys()):
        funcsym[s] = list()
        strsym[s] = list()

    ignore = dict()  # Ignore strings by state
    errorf = dict()  # Error functions by state
    ...
    for state in list(stateinfo.keys()):
        ...
        for fname, f in funcsym[state]:
            ...
            if f.__doc__:
                if not optimize:
                    try:
                        c = re.compile("(?P<%s>%s)" % (f.__name__, f.__doc__),
                                       re.VERBOSE | reflags)
                        if c.match(""):
                            print(
        ...
        for name, r in strsym[state]:
            ...
            if not optimize:
                ...
                if tokname not in lexobj.lextokens and tokname.find(
                        "ignore_") < 0:
                    print(
                ...
                try:
                    ...
                    print(
                        "lex: Invalid regular expression for rule '%s'. %s" % (
                            name, e))
                    if '#' in r:
                        print(
