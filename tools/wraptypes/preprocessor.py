    ...
    def p_include_line(self, p):
        ...
        def p_define_object(self, p):
            ...
            if p.parser.enable_declaratives():
                ...
                if value is not None:
                    ...
                    p.parser.write(
                        (create_token('PP_DEFINE', (p[2], value), p),))
        ...
        class PreprocessorParser(yacc.Parser):
            ...
            def __init__(self, gcc_search_path=True):
                ...
                # Map system header name to data, overrides path search and
                # open()
                self.system_headers = dict()
            ...
            def parse(self, filename=None, data=None, namespace=None,
                      debug=False):
                self.output = list()
                if not namespace:
                    namespace = self.namespace
                for name, value in list(self.defines.items()):
                    namespace.define_object(name, (
                        create_token('IDENTIFIER', value),))
            ...
            def include(self, header):
                ...
                def include_system(self, header):
                    ...
                    if path:
                        ...
                        print(
                            '"%s" not found' % header, file=sys.stderr)  # TODO

                    def include_next(self, header, reference):
                        # TODO: doesn't go via get_system_header
                        next = False
                        ...
                        def import_(self, header):
                            ...
                            def import_system(self, header):
                                ...
                                def get_header_path(self, header):
                                    p = os.path.join(
                                        os.path.dirname(self.lexer.filename),
                                        header)
                                    if os.path.exists(p):
                                        self.push_file(p)
                                        return p
                                    elif sys.platform == 'darwin':
                                        p = self.get_framework_header_path(
                                            header)
                                        if not p:
                                            p = self.get_system_header_path(
                                                header)
                                        return p
                                ...
                                def get_system_header_path(self, header):
                                    for path in self.include_path:
                                        ...
                                        if os.path.exists(p):
                                            return p
                                    if sys.platform == 'darwin':
                                        return self.get_framework_header_path(
                                            header)

                                def get_framework_header_path(self, header):
                                    if '/' in header:
                                        # header is 'Framework/Framework.h'
                                        # (e.g. OpenGL/OpenGL.h).
                                        framework, header = header.split(
                                            '/', 1)

                                        paths = self.framework_path[:]
                                        # Add ancestor frameworks of current
                                        # file
                                        localpath = ''
                                        for parent in self.lexer.filename.split(
                                                '.framework/')[:-1]:
                                            localpath += parent + '.framework'
                                            paths.append(os.path.join(localpath,
                                                                      'Frameworks'))
                                        for path in paths:
                                            p = os.path.join(path,
                                ...
                                def error(self, message, filename, line):
                                    print('%s:%d #error %s' % (
                                        filename, line, message), file=sys.stderr)

                                    def condition_if(self, result):
                                        self.condition_stack.append(
                                            ExecutionState(self.condition_stack[
                                                -1].enabled,
                                                result))
                                    ...
                                    def enable_elif_conditionals(self):
                                        return self.condition_stack[
                                            -1].parent_enabled and \
                                            not self.condition_stack[
                                            -1].context_enabled

                                    def write(self, tokens):
                                        for t in tokens:
                                            if t.type == 'HEADER_NAME':
                                                # token was mis-parsed.  Do it
                                                # again, without the '<', '>'.
                                                ta = create_token('<', '<')
                                    ...
                                    def get_memento(self):
                                        return (
                                            set(self.namespace.objects.keys()),
                                ...
                                class ConstantExpressionParser(yacc.Parser):
                                    ...
                                    def __init__(self, lexer, namespace):
                                        ...
                                        self.lexer = lexer
                                        self.namespace = namespace
                                        self._const_grammar.get_prototype().init_parser(
                                            self)

                                    def parse(self, debug=False):
                                        self.result = None
                                        try:
                                            yacc.Parser.parse(self,
                                                              lexer=self.lexer,
                                                              debug=debug)
                                ...
                                class StrictConstantExpressionParser(
                                ...
                                class PreprocessorNamespace(EvaluationContext):
                                    ...
                                    def add_stdc_macros(self):
                                        ...
                                        date = time.strftime(
                                            '%b %d %Y')  # TODO: %d should have leading space
                                    ...
                                    def add_gcc_macros(self):
                                        ...
                                        gcc_macros = (
                                            '__GLIBC_HAVE_LONG_LONG', '__GNUC__',)
                                        ...
                                        machine_macros = {
                                            'x86_64': (
                                                ...
                                                '__x86_64__',
                                                '__tune_k8__', '__MMX__', '__SSE__',
                                                '__SSE2__',
                                                '__SSE_MATH__', '__k8', '__k8__'),
                                            'Power Macintosh': (
                                                '_ARCH_PPC', '__BIG_ENDIAN__',
                                                '_BIG_ENDIAN',
                                                '__ppc__', '__POWERPC__'),
                                            # TODO everyone else.
                                        ...
                                        self.define_object('__const', (
                                            create_token('IDENTIFIER', 'const'),))
                                    ...
                                    def define_object(self, name, replacements):
                                        # TODO check not already existing in
                                    ...
                                    def define_function(self, name, params,
                                        ...
                                        self.functions[
                                            name] = replacements, numargs

                                    def apply_macros(self, tokens,
                                                     replacing=None):
                                        repl = list()
                                        i = 0
                                        while i < len(tokens):
                                            ...
                                            if token.type == 'IDENTIFIER' and token.value in self.objects:
                                                ...
                                                r, numargs = self.functions[
                                                    token.value][:]

                                                # build params list
                                                i += 2
                                                params = [list()]
                                                # balance parantheses within
                                                # each arg
                                                parens = 0
                                                while i < len(tokens):
                                                    if tokens[
                                                            i].value == ',' and parens == 0 and \
                                                                        len(
                                                            params) < numargs:
                                                        params.append(list())
                                                    elif tokens[
                                                            i].value == ')' and parens == 0:
                                                        break
                                                    else:
                                                        if tokens[
                                                                i].value == '(':
                                                ...
                                                if len(tokens) - i > 3 and \
                                                        ...
                                                        i + 1].type in (
                                                            '(', 'LPAREN') and \
                                                        tokens[
                                                        i + 2].type == 'IDENTIFIER' and \
                                                        tokens[
                                                        i + 3].type == ')':
                                                    result = self.is_defined(
                                                        tokens[i + 2].value)
                                                    i += 3
                                    ...
                                    def copy(self):
                                        n = PreprocessorNamespace(
                                            gcc_macros=False,
                                            workaround_macros=False)
