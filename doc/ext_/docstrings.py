...
def modify_docstrings(app, what, name, obj, options, lines,
    ...
    while i < len(lines):
        ...
        if ":parameters:" in line.lower():
            ...
            lines[i] = lines[i].replace(':guide:`',
                                        '.. seealso:: Programming Guide - :ref:`guide_')

        elif ":deprecated:" in line:
            lines[i] = lines[i].replace(':deprecated:',
                                        '.. warning:: Deprecated.')
            lines.insert(i, "")
            lines.insert(i, "")

        elif line.strip().startswith(":since:"):
            lines[i] = lines[i].replace(':since:',
                                        '.. note:: Since')
            lines.insert(i + 1, "")
            lines.insert(i, "")
            ...
            lines[i] = lines[i].replace(':event:', '.. event mark')
