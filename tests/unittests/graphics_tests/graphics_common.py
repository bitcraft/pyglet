...
def get_feedback(func):
    ...
    glFeedbackBuffer(
        len(feedback_buffer), GL_4D_COLOR_TEXTURE, feedback_buffer)
...
class GraphicsGenericTestCase:
    ...
    def setUp(self):
        ...
        self.n_vertices = 36
        self.v3f_data = [v / float(self.n_vertices * 3) for v in
                         range(self.n_vertices * 3)]
        self.v2f_data = [v / float(self.n_vertices * 2) for v in
                         range(self.n_vertices * 2)]
        self.c4f_data = [v / float(self.n_vertices * 4) for v in
                         range(self.n_vertices * 4)]
        self.c3f_data = [v / float(self.n_vertices * 3) for v in
                         range(self.n_vertices * 3)]
        self.t4f_data = [v / float(self.n_vertices * 4) for v in
                         range(self.n_vertices * 4)]
        self.t3f_data = [v / float(self.n_vertices * 3) for v in
                         range(self.n_vertices * 3)]
        self.t2f_data = [v / float(self.n_vertices * 2) for v in
                         range(self.n_vertices * 2)]
    ...
    def check(self, expected, result, dimensions):
        ...
        if len(expected) != len(result) * dimensions / 4:
            self.fail('Incorrect number of vertices in feedback array: '
                      'expected: %d, obtained: %d' % (
                          len(expected), len(result) * dimensions / 4))

        # there are two factors here:
        # 1. there are several triangles and their out-order can be different from in-order.
        # 2. in each triangle, the out-order of the vertices can be a rotation
        # from its in-order.
        ...
        # to tackle 2:
        # within each triangle, we cycle the vertices list and check if any
        # rotation matches the expected.
        ...
        for e_chunk in chunks(expected, dimensions):
            ...
            for result_chunk in chunks(result, 3 * 4):
                # here we tackle 2.
                if self._are_equal_within_rotation(expected_chunk,
                                                   result_chunk):
                    was_found = True
                    break
    ...
    def test_v2f_t2f(self):
        self.generic_test('v2f', self.v2f_data, None, None, 't2f',
                          self.t2f_data)

    def test_v3f_c3f_t2f(self):
        self.generic_test('v3f', self.v3f_data, 'c3f', self.c3f_data, 't2f',
                          self.t2f_data)

    def test_v3f_c3f_t3f(self):
        self.generic_test('v3f', self.v3f_data, 'c3f', self.c3f_data, 't3f',
                          self.t3f_data)

    def test_v3f_c4f_t4f(self):
        self.generic_test('v3f', self.v3f_data, 'c4f', self.c4f_data, 't4f',
                          self.t4f_data)
