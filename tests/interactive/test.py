...
class TestCase:
    ...
    def test(self, options):
        ...
        if num_failures or num_errors:
            print(
                '%d Failures and %d Errors detected.' % (
...
class TestPlan:
    ...
    def run(self, options, names=list()):
        if not names:
            ...
            for name in names:
                if name not in self.names:
                    options.log.error(
                        'Unknown test case or section "%s"', name)
                    return False
                else:
...
class RegressionCaptureTestResult(unittest.TestResult):
    ...
    def startTest(self, test):
        ...
        if isinstance(test, unittests.regression.ImageRegressionTestCase):
...
class RegressionCheckTestResult(unittest.TestResult):
    ...
    def addSuccess(self, test):
        # Check image
        ref_image = self.regression_image.get_image_data()
        this_image = test._captured_image.get_image_data()
    ...
    def addFailure(self, test, err):
        err = Regression(err)
        super().addFailure(test, (Regression,
                                  err, list()))
