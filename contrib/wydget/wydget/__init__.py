...
class GUI(event.GUIEventDispatcher):
    ...
    # TODO: I suspect that this is duplicating functionality in layout
        ...
        if self.debug:
            ...
            for o, (x, y, z, c) in rects:
                ...
                if o.view_clip:
                    v = o.view_clip
                    glColor4f(0, 0, 1, .1)
                    glRectf(x + v.x, y + v.y, x + v.x + v.width,
                            y + v.y + v.height)

            glDisable(GL_BLEND)
            self.debug_display.draw(0, 0, 0, util.Rect(0, 0,
                                                       self.width,
                                                       self.debug_display.height))
