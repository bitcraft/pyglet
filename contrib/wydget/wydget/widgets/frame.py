...
class ContainerFrame(FrameCommon):
    ...
    def checkForScrollbars(self):
        ...
        # TODO: perhaps this should be on the parent
        ...
        if v_needed:
            ...
            if p.v_slider is not None:
                # TODO: update the range more sanely
                p.v_slider.range = r
            else:
                p.v_slider = VerticalSlider(p, 0, r, r, x=vc_width, y=y,
                                            height=h, step=16,
                                            classes=('-frame-vertical-slider',))
                change = True
        ...
        if h_needed:
            ...
            # if change:
            # TODO: really do need to do something here to resize contents
        return change
...
class TabsLayout(layouts.Layout):
    ...
    def getChildren(self):
        ...
        """
        return [c for c in super().getChildren()
                if not c.scrollable]
...
class TabbedFrame(Frame):
    ...
    def newTab(self, text=None, image=None, border=default,
        ...
        if border is self.default:
            border = self.border
        if bgcolor is self.default:
        ...
        if scrollable:
            ...
            r = f = self.frame_class(self.bottom, border=border, x=0, y=0,
                                     bgcolor=bgcolor, padding=2, height='100%',
                                     width='100%')
        b._frame = f
        f._button = b
