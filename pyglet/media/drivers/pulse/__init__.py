...
class PulseAudioDriver(AbstractAudioDriver):
    ...
    def connect(self, server=None):
        ...
        self._context = pa.pa_context_new(
            self.mainloop, app_name.encode('ASCII'))
...
class PulseAudioPlayer(AbstractAudioPlayer):
    ...
    def __init__(self, source_group, player):
        ...
        self._events = list()
        self._timestamps = list()  # List of (ref_time, timestamp)
        self._write_index = 0  # Current write index (tracked manually)
        # True only if buffer has non-stale data
    ...
    def _underflow_cb(self, stream, data):
        ...
        if self._underflow_is_eos:
            ...
            # TODO:: sometimes receive an underflow after EOS... need to
            # filter?
