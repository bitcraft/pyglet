...
iokit.IOHIDDeviceCopyMatchingElements.argtypes = [
    c_void_p, c_void_p, IOOptionBits]
...
iokit.IOHIDDeviceGetTypeID.argtypes = list()
...
iokit.IOHIDDeviceRegisterInputValueCallback.argtypes = [
...
iokit.IOHIDElementGetTypeID.argtypes = list()
...
iokit.IOHIDManagerGetTypeID.argtypes = list()
...
iokit.IOHIDValueGetTypeID.argtypes = list()
...
_device_lookup = dict()  # IOHIDDeviceRef to python HIDDevice object
_element_lookup = dict()  # IOHIDElementRef to python HIDDeviceElement object
...
class HIDDevice:
    ...
    def get_property(self, name):
        cfname = CFSTR(name)
        cfvalue = c_void_p(
            iokit.IOHIDDeviceGetProperty(self.deviceRef, cfname))
        cf.CFRelease(cfname)
    ...
    def get_value(self, element):
        ...
        iokit.IOHIDDeviceGetValue(
            self.deviceRef, element.elementRef, byref(valueRef))
        if valueRef:
...
class PygletDevice(Device):
    ...
    def __init__(self, display, device, manager):
        super().__init__(display, device.product)
        self.device = device
    ...
    def _create_controls(self):
        ...
        for element in self.device.elements:
            raw_name = element.name or '0x%x:%x' % (
