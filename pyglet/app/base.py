...
class EventLoop(event.EventDispatcher):
    ...
    def idle(self):
        ...
        The method is called immediately after any window events (i.e.,
        after any user input).  The method can return a duration after which
        the idle method will be called again.  The method may be called
        earlier if the user creates more input events.  The method can return
        `None` to only wait for user events.

        For example, return ``1.0`` to have the idle method called every second,
        or immediately after any user events.
        ...
        This method should be overridden by advanced users only.  To have code
        execute at regular intervals, use the `pyglet.clock.schedule` methods.
    ...
    def has_exit(self):
        ...
        When set, all waiting threads are interrupted (see `sleep`).
    ...
    def exit(self):
        ...
        ``True``.  All waiting threads will be interrupted (see `sleep`).
        """
        self.has_exit = True
